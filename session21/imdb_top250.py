...
def parse_html(html):
    ...
    for movie_item in movie_list.find_all("li"):
        ...
        # Get rating
        rating = metadata[-1].string

        # Get reviews
        reviews = summary.find('span', attrs = 
        {"class": "ipc-rating-star"}).get_text()
        # print(reviews)

        reviews, vote_count = reviews.split()
        vote_count = vote_count.strip('()')
        print(reviews, vote_count)

        top_250.append((title, year, length, rating, reviews))
...
def main():
    ...
    with open("data/imdb_top250.csv", "w", encoding="utf-8", newline="") as f:
        ...
        fields = ("title", "year", "length", "rating", "reviews")
